...
<button onclick="activateInnerTab(this); showBowling('${name}')">BOWLING</button>
</div>
...
document.getElementById('content').innerHTML=`
<div class="subTabs">
<button class="active" onclick="activateInnerTab(this); loadBattingBoard()">BATTING</button>
<button onclick="activateInnerTab(this); loadBowlingBoard()">BOWLING</button>
<button onclick="activateInnerTab(this); loadMVP()">MVP</button>
</div>
<div id="board"></div>`;
loadBattingBoard();
}
...
if(!battingData.length || !bowlingData.length){
    document.getElementById('content').innerHTML="<h3>Loading data...</h3>";
    return;
}
...
function activateInnerTab(button){
...
document.addEventListener("DOMContentLoaded", ()=>{
    document.querySelectorAll(".mainTabs a").forEach(a=>{
        a.addEventListener("click", ()=>{
            // Normal href navigation is intentionally preserved.
        });
    });
